...
local BlockedSpells = T {24931}

local BlockedBuffs = T {0, 1, 603, 627}
...
local MultipleAllowed = T {309, -- Corsair
...
local function InsertBuff(actor, target, effect, name, duration, replace, time)
    ...
    if (not target or target == 0) or (not actor or actor == 0) then
        return
    end

    if not effect or effect == 0 or BlockedBuffs:hasval(effect) then
        return
    end
...
local function HandleCorsairRoll(target, effect, spell, actor)
    ...
    if a.AreaRange > 0 then
        SetAoEBuff(name, effect, duration, o_time)
    end
...
local function HandleAction(act)
    ...
    if #act.targets == 0 then
        return
    end
    if #act.targets[1].actions == 0 then
        return
    end
    ...
    if not BlockedSpells:hasval(spell) and
        (AbilityMsgs:haskey(message) or SpellMsgs:haskey(message) or PetMsgs:haskey(message)) then
...
local function HandlePartyBuffUpdate(e)
    ...
    for i = 0, 4 do
        ...
        if memberIndex > 0x6FF and memberIndex <= 0x8FF then
            goto continue
        end
    ...
    if not AoEBuff.id or not AoEBuff.show then
        return
    end
    ...
    for target, buff_t in pairs(packetBuffs) do
        for _, v in pairs(buff_t) do
            if v == AoEBuff.id then
                ...
                InsertBuff(actor, target, v, AoEBuff.name, AoEBuff.duration, false, AoEBuff.o_time)
            end
        end
    end
...
function HandleBuffUpdate(e)
    ...
    if not p_server_id or not buffsByCharacter[p_server_id] then
        return
    end
            ...
            for effect_copy_index, effect_info in pairs(effects_table) do
                ...
                if (render) then
                    table.insert(bars, {
                        ...
                        remains = time_remaining,
                        key = effect_info.t_index .. effect_id,
                        icon_id = effect_id
                    })
                end
